"""axiom.olap.clickhouse.repository.mutation.async_ — Asynchronous ClickHouse mutation manager."""
...
class AsyncClickHouseMutationManager:
    """Asynchronous manager for monitoring and controlling ClickHouse mutations."""

    def __init__(self, client: Any) -> None:
        """Initialize the async mutation manager.
    ...
    async def get_mutation(self, mutation_id: str) -> MutationStatus | None:
...
"""
        try:
            result = await self._client.query(query, parameters={"mid": mutation_id})
    ...
    async def wait_for_mutation(
        ...
        while asyncio.get_event_loop().time() < deadline:
            status = await self.get_mutation(mutation_id)
            if status is None:
    ...
    async def kill_mutation(self, mutation_id: str) -> None:
        """Kill a running mutation.

        Use with caution. Killing an active mutation may leave data in inconsistent state.
        Always verify mutation status before killing.
    ...
    async def list_stuck_mutations(
        self,
        table: str,
        database: str | None = None,
        ...
        """
        all_active = await self.list_mutations(table, database, active_only=True)
    ...
    @classmethod
    async def from_settings(cls, settings: ClickHouseSettings) -> Self:
        """Create an async mutation manager from ClickHouseSettings.

        Args:
            settings: ClickHouse connection settings.

        Returns:
            New AsyncClickHouseMutationManager instance.
        """
