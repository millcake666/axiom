"""axiom.olap.clickhouse.repository.schema.sync_ — Synchronous ClickHouse schema manager."""
...
class ClickHouseSchemaManager:
    """Synchronous manager for ClickHouse DDL and schema introspection."""
    ...
    def _resolve_database(self, database: str | None) -> str | None:
        ...
        Args:
            database: Override database or None to use instance default.

        Returns:
            Effective database name or None.
        """
    ...
    def describe_table(self, table: str, database: str | None = None) -> TableInfo:
        """Describe a table, returning column metadata and DDL.
        ...
        try:
            ...
            ddl = self.get_create_table_ddl(table, database)
            engine_result = self._client.query(
    ...
    def get_create_table_ddl(self, table: str, database: str | None = None) -> str:
        ...
        qualified = self._qualified(table, database)
        try:
            result = self._client.command(f"SHOW CREATE TABLE {qualified}")
            return str(result)
        except Exception as exc:
            raise ClickHouseSchemaError(str(exc)) from exc
    ...
    def drop_table(
        ...
        try:
            self._client.command(f"DROP TABLE {clause}{qualified}")
        except Exception as exc:
            raise ClickHouseSchemaError(str(exc)) from exc
    ...
    def truncate_table(self, table: str, database: str | None = None) -> None:
        ...
        qualified = self._qualified(table, database)
        try:
            self._client.command(f"TRUNCATE TABLE {qualified}")
        except Exception as exc:
            raise ClickHouseSchemaError(str(exc)) from exc
    ...
    def optimize_table(
        ...
        """Run OPTIMIZE TABLE to force merging of parts.

        Args:
            table: Table name.
            database: Optional database override.
            final: If True, add FINAL keyword to force single-part merge.
        ...
        if deduplicate:
            suffix += " DEDUPLICATE"
        try:
            self._client.command(f"OPTIMIZE TABLE {qualified}{suffix}")
        except Exception as exc:
            raise ClickHouseSchemaError(str(exc)) from exc
    ...
    def from_settings(
        ...
        """Create a schema manager from ClickHouseSettings.
        ...
        Returns:
            New ClickHouseSchemaManager instance.
        """
