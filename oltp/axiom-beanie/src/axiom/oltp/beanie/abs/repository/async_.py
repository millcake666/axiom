...
"""axiom.oltp.beanie.abs.repository.async_ — Abstract async repository for Beanie."""
...
class AsyncBaseRepository[ModelType: Document, SessionType, QueryType](ABC):
    ...
    def __init__(self, model: type[ModelType], db_session: SessionType) -> None:
        ...
        Args:
            ...
            db_session: The database session or client to use for operations.
        """
    ...
    def _maybe_join(self, query: QueryType, field: str) -> QueryType:
        """Optionally extend the query to fetch linked documents for ``field``.

        Args:
            query: The current query object.
            field: The field path that may require a join/fetch.

        Returns:
            The query, potentially extended with a join.
        """
        raise NotImplementedError
    ...
    def _validate_params(self, field: str, value: Any | None = None) -> None:
        """Validate a value against a field's type, supporting nested paths.
        ...
        Raises:
            ValidationError: If the field path is invalid or type mismatch.
        """
        from axiom.core.exceptions import ValidationError
        from axiom.oltp.beanie.base.utils import validate_nested_field

        try:
            validate_nested_field(self.model_class, field, value)
