...
| [`axiom-fastapi`](./axiom-fastapi/README.md) | `axiom.fastapi` | рабочий | FastAPI app factory, middleware, handlers, docs routes, runners, rate limiting |
...
- встроенного rate limiting для FastAPI через in-memory или Redis backend;
...
- `axiom-fastapi` все еще смешивает logging stacks: новый rate limiter использует `axiom.core.logger`, а legacy middleware/handlers еще не полностью мигрировали со `structlog`;
