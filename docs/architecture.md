...
- можно подключить `axiom-fastapi`, но не использовать MongoDB или S3;
...
3. `ErrorMiddleware` ловит необработанные исключения.
4. endpoint вызывает controller.
5. controller делегирует repository.
6. repository применяет `FilterRequest` и делает запрос к БД.
7. controller оборачивает список в `PaginationResponse` или поднимает `NotFoundError`.
8. handlers сериализуют `BaseError` в `ErrorDetail`.
...
- частично реализованы: `axiom-fastapi` без rate limiter;
